...
describe("treasury-wallet", () => {
  ...
  before(async () => {
    ...
    const [_treasuryAuthorityPubkey, _treasuryAuthorityBump] =
      PublicKey.findProgramAddressSync(
        [treasuryWallet.publicKey.toBuffer()],
        program.programId
      );

    treasuryWalletAuthorityPda = {
      publicKey: _treasuryAuthorityPubkey,
      bump: _treasuryAuthorityBump,
    };
    ...
    await sendAndConfirmTransaction(provider.connection, mintToTx, [owner], {
      commitment: "confirmed",
    });
  ...
  it("happy flow", async () => {
    ...
    console.log(
      `initialized treasury wallet: ${treasuryWallet.publicKey.toBase58()}`
    );
    ...
    const [_withDrawAuthorizationPubkey, _withDrawAuthorizationBump] =
      PublicKey.findProgramAddressSync(
        [
          treasuryWallet.publicKey.toBuffer(),
          withdrawAuthority.publicKey.toBuffer(),
        ],
        program.programId
      );

    const withdrawAuthorizationPda = {
      publicKey: _withDrawAuthorizationPubkey,
      bump: _withDrawAuthorizationBump,
    };
    ...
    console.log(
      `adding withdraw authority: ${withdrawAuthority.publicKey.toBase58()}`
    );
    await sendAndConfirmTransaction(
      provider.connection,
      addWithdrawAuthorityTx,
      [owner],
      { commitment: "confirmed" }
    );
    console.log(
      `added withdraw authority: ${withdrawAuthority.publicKey.toBase58()}`
    );
    ...
    console.log(
      `withdrawing tokens to withdraw authority token account: ${withdrawAuthorityATA.toBase58()}`
    );
    ...
    const withdrawAuthorityTokenAccount =
