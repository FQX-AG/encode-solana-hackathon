...
describe("transfer-snapshot-hook", () => {
  const program = anchor.workspace.TransferSnapshotHook as Program<TransferSnapshotHook>;
  ...
  it("setup extra account metas", async () => {
    ...
    const initExtraAccountMetaInstruction = await program.methods
      .initializeExtraAccountMetaList(_bump, 64)
      .accounts({
